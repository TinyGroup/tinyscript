...
public class Example6Test extends TestCase {
	...
	public void test6WithScript() throws Exception {
		...
		GroupDataSet groupDs = (GroupDataSet) engine
				.execute("m = new Example6(); return m.countStock(\"src/test/resources/StockRecords.txt\");", context);
		// 输出结果股票
		for (int i = 0; i < groupDs.getRows(); i++) {
			System.out.println("code=" + groupDs.getData(groupDs.isIndexFromOne() ? i + 1 : i, 1));
		}
		System.out.println("test6WithScript is end!");
	}

	public void testMailCharge() throws Exception {
		DataSet resultDs = (DataSet) engine.execute(
				"m = new Example6(); return m.countMailCharge(\"src/test/resources/mailCharge.txt\",\"src/test/resources/testOrder.txt\");",
				context);
		// 输出邮费
