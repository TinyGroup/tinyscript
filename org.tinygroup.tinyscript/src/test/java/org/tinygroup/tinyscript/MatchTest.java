...
public class MatchTest extends TestCase {
	...
	public void  testMatch() throws Exception {
		...
		//匹配花费规则
		DataSet result1 = (DataSet) engine.execute(" costRuleDs = ruleDs.filter(FIELD == \"COST\").sort(\"MINVAL\"); return orderDs.match(costRuleDs,COST > MINVAL);",context);
		...
		//匹配重量规则
		DataSet result2 = (DataSet) engine.execute(" weightRuleDs = ruleDs.filter(FIELD == \"WEIGHT\").sort(\"MINVAL\"); return orderDs.match(weightRuleDs,\"WEIGHT >= MINVAL \"+\"&& WEIGHT <MAXVAL\");",context);
