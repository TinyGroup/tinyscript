...
public abstract class DynamicNameScriptFunction extends AbstractScriptFunction {
	 ...
	 * @throws Exception
	 */
	protected Object[] subArray(Object[] oldParams, int start) throws Exception {
		Object[] newParams = new Object[oldParams.length - start];
		for (int i = start; i < oldParams.length; i++) {
			newParams[i - start] = getValue(oldParams[i]);
		}
		return newParams;
	}
