...
public abstract class TwoConvertOperator extends TwoOperator {
    ...
    public Object operation(Object... parameter) throws ScriptException {
    	...
    	if(ExpressionUtil.isNumber(left.getClass()) && ExpressionUtil.isNumber(right.getClass())){
    	   //如果两个都是数字类型
    ...
    private Object operateNumber(Object left, Object right, Class type1, Class type2) {
    	...
    	if (!type1.equals(type2)) {
    	   if(ExpressionUtil.compare(type1, type2)>0){
    		   rightObject = ExpressionUtil.convert(rightObject, type2, type1);
    	   }else{
    		   leftObject = ExpressionUtil.convert(leftObject, type1, type2);
    	   }
    	}
