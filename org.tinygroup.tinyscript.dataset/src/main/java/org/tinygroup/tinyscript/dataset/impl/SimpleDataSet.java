...
public class SimpleDataSet extends DynamicDataSet implements Cloneable {
	...
	private void checkValidate() throws Exception {
		if (dataArray != null && dataArray.length == 0) {
			throw new Exception(ResourceBundleUtil.getResourceMessage("dataset", "dataset.row.notexist", "1"));
		}
	}
	...
	public boolean previous() throws Exception {
		checkValidate();
		if (currentRow == 0) {
			throw new Exception(ResourceBundleUtil.getResourceMessage("dataset", "dataset.row.move.error", "0"));
		}
		currentRow--;
		return true;
	}
	...
	public boolean absolute(int row) throws Exception {
		...
		if (isIndexFromOne()) {
			if (row < 1 || row >= dataArray.length + 1) {
				throw new Exception(
						ResourceBundleUtil.getResourceMessage("dataset", "dataset.row.outofindex", "absolute", row));
			}
		} else {
	...
	public DynamicDataSet deleteRow(int row) throws Exception {
		...
		if (isIndexFromOne()) {
			if (row < 1 || row >= rowNum + 1) {
				throw new Exception(
						ResourceBundleUtil.getResourceMessage("dataset", "dataset.row.outofindex", "deleteRow", row));
			}
		} else {
			if (row < 0 || row >= rowNum) {
				throw new Exception(
	...
	public DynamicDataSet insertRow(int row) throws Exception {
		...
		if (isIndexFromOne()) {
			if (row < 1 || row > rowNum + 1) {
				throw new Exception(
						ResourceBundleUtil.getResourceMessage("dataset", "dataset.row.outofindex", "insertRow", row));
			}
		} else {
			if (row < 0 || row > rowNum) {
				throw new Exception(
