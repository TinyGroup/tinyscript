...
public class DataSetGroupDynamicFunction extends AbstractGroupFunction {
	...
	private GroupDataSet groupDynamic(DynamicDataSet dataSet, String expression, ScriptContext context)
			throws Exception {
		try {
	...
	private List<DynamicDataSet> group(AbstractDataSet dataSet, String expression, ScriptContext context)
			throws Exception {
			...
			for (int i = 0; i < rowNum; i++) {
				...
				if (groupDataSet == null) {
					...
					for (int j = 0; j < groupDataSet.getColumns(); j++) {
						groupDataSet.setData(dataSet.getShowIndex(row), dataSet.getShowIndex(j),
								dataSet.getData(dataSet.getShowIndex(i), dataSet.getShowIndex(j)));
					}
				}
			}
