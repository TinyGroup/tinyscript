...
public class WriteSqlFunction extends AbstractScriptFunction {
	...
	private Object insertDataSet(AbstractDataSet ds,String table,DataSource dataSource,ScriptContext context) throws Exception{
		DataSource newDataSource = dataSource==null?(DataSource)ScriptUtil.getVariableValue(context, ScriptContextUtil.getCustomBeanName(context)):dataSource;
		...
		try{
			...
			throw new ScriptException(String.format("%s函数执行SQL[%s]发生异常:", getNames(),insertSql), e);
		}finally{
			if(ps!=null){
			   ps.close();
			}
	...
	private Map<Integer,Integer> createMap(DataSet ds,ResultSetMetaData rsmd) throws Exception{
		...
		for(int i=0;i<fields.size();i++){
			...
			if(!maps.containsKey(name)){
				throw new ScriptException(String.format("目标表不包含列[%s]", f.getName()));
			}
